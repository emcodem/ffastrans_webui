...
module.exports = {
  fetchjobs: function () {    
    ...
    Request.get(buildApiUrl(global.config.STATIC_GET_RUNNING_JOBS_URL), {timeout: 1000},(error, response, body) => {
        if(error) {
    ...
    Request.get(buildApiUrl(global.config.STATIC_GET_FINISHED_JOBS_URL), {timeout: 1000},(error, response, body) => {
        if(error) {
            global.socketio.emit("error", 'Error, webserver lost connection to ffastrans server. Is FFAStrans API online? ' + buildApiUrl(global.config.STATIC_GET_QUEUED_JOBS_URL));
            return;
        }
