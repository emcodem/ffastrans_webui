/* https://github.com/ShinobiCCTV/jsmpeg-pipe/blob/master/jsmpeg.pipe.js */
var allAudioBuffer;
        ...
        if (this.audio) {
            ...
            do {
                if (this.audioOut.enqueuedTime > this.maxAudioLag) {
                    console.log("reset enqueuedtime")
                    this.audioOut.resetEnqueuedTime();
                    this.audioOut.enabled = false
                }
                
                let decoded = this.audio.decode()
                // if (decoded){
...
JSMpeg.Demuxer.TS = function() {
    ...
    TS.prototype.parsePacket = function() {
        ...
        if (this.bits.read(8) !== 71) {
            this.resync=TS.prototype.resync
            if (!this.resync()) {
                return false
            }
        }
...
JSMpeg.AudioOutput.WebAudio = function() {
    ...
    WebAudioOut.prototype.play = function(sampleRate, left, right) {
        ...
        if (!this.enabled) {
            console.error("audio is not enabled")
            return
        }
        if (!this.unlocked) {/* apple only */
            var ts = JSMpeg.Now();
            if (this.wallclockStartTime < ts) {
                this.wallclockStartTime = ts
            }
            this.wallclockStartTime += left.length / sampleRate;
            console.error("skipped audio sample, duration:",left.length/sampleRate)
            return
        }
        ...
        // if (!allAudioBuffer)
        //     allAudioBuffer = new Float32Array(0);
        // allAudioBuffer=   new Float32Array([...allAudioBuffer, ...left]);
        ...
        if (this.startTime < now) { //is starttime in the past? starttime is now the starttime + duration from last run
            console.log("starttime < now,",this.startTime,now)
            this.startTime = now;
            this.wallclockStartTime = JSMpeg.Now()
            this.startTime += duration; //next startTime
            this.wallclockStartTime += duration
            return
        }

        if (this.lastDuration != 0){
            if ((this.lastStartTime + this.lastDuration) > this.startTime){
                let diff = this.startTime - (this.lastStartTime + this.lastDuration)
                console.log("Audio Overlapping, diff: ",diff)
            }
            if (this.startTime > (this.lastStartTime + this.lastDuration)){
                let gap = this.startTime - (this.lastStartTime + this.lastDuration);
                console.log("Audio Gap, size: ",gap)
            }
        }

        source.start(this.startTime); //could start in the future
        //console.log("startTime",this.startTime,"wallclockStartTime",this.wallclockStartTime,"duration",duration)
