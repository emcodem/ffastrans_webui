...
TS.prototype.packetComplete = function(pi) {
	...
	//try and buffer video until we got matching audio
	...
	if (pi.destination.constructor.name == "MP2"){
		//audio goes out immediately
		reporting.aud += pi.destination.left.length;
		if (!firstSeenAudioPts){
			firstSeenAudioPts = pi.pts;
		}

	}
	if (pi.destination.constructor.name == "MPEG1"){
		reporting.vid += 1
		if (!firstSeenAudioPts && videoPiBuffer.length < 10){
			//buffer video until audio matches (or buffer too large)
			console.warn("video too early, buffering")
			videoPiBuffer.push(pi);
			return;
		}else{
			gotAudioSync = true;
		}
