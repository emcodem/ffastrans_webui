...
(function($){ 	
	...
	*/
	let syncColsTimer;
	let nonrenderedDragDistance ={x:0,l:0}

	var syncCols = function(t,i,isOver){
		//without storing drag.x and drag.l in our own scoped object, resizing was a little chunky sometimes
			nonrenderedDragDistance.x = drag.x;//what the hell is the scope of drag, it is not in this!	
			nonrenderedDragDistance.l = drag.l;//what the hell is the scope of drag, it is not in this!	
			
			if (syncColsTimer){
				clearTimeout(syncColsTimer);
			}
			let _drag = drag;
			syncColsTimer = window.setTimeout(function(nonrenderedDragDistance){
				let inc = nonrenderedDragDistance.x - nonrenderedDragDistance.l;
				let c = t.c[i];
				let c2 = t.c[i+1]; 	
				nonrenderedDragDistance.x = 0;
				nonrenderedDragDistance.l = 0;
				let w = c.w + inc;	var w2= c2.w- inc;	//their new width is obtained					
				c.width( w + PX);			
		...
		},1,nonrenderedDragDistance);//timeout was a try to prevent too many redraws/reflows but doesnt work because we need to collect dragx and dragl when discarding the stuff
	...
	*/
	...
	var onGripDrag = function(e){	
		if(!drag) return; 
        let t = drag.t;		//table object reference 
        var oe = e.originalEvent.touches;
        var ox = oe ? oe[0].pageX : e.pageX;    //original position (touch or mouse)
        var x =  ox - drag.ox + drag.l;	        //next position according to horizontal mouse position increment
		let mw = t.opt.minWidth, i = drag.i ;	//cell's min width
