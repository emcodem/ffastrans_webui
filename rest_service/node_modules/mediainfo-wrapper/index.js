...
module.exports = function MediaInfo(file, htmlOutput) {
	var exec_options = {};
    var cmd = [];
    cmd.push(getCmd()); // base command e.g. c:\node_modules\mediainfo.exe
	if (htmlOutput){
		cmd.push('--Output=HTML ');
	}else{
		cmd.push('--Output=JSON '); 
	}
    // Array.prototype.slice.apply(args).forEach(function (val, idx) {
        // var files = glob.sync(val, {cwd: (exec_options.cwd || process.cwd()), nonull: true});//finds files in a folder?
        // console.log("Minfo files", files)
        // for (var i in files) {
	console.log("FILEFILE",file)
    cmd.push(safeLocalPath(file)); // files
        // }
    // });
	console.log("cmd",cmd)
    return new Promise(function (resolve, reject) {
        console.log("exec",cmd)
        exec(cmd.join(' '), exec_options, function (error, stdout, stderr) {
